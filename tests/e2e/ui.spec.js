...
test.describe('All test for realworld', () => {
  ...
  test('User authorization @e2e', async ({ webApp }) => {
  ...
  test('Create new article - positiv @e2e', async ({ webApp }) => {
    const user = JSON.parse(fs.readFileSync('user.json', 'utf8'));
  ...
  test('Create new article - negativ @e2e', async ({ webApp }) => {
    const user = JSON.parse(fs.readFileSync('user.json', 'utf8'));
  ...
  test('Click popular tags @e2e', async ({ webApp }) => {
    const user = JSON.parse(fs.readFileSync('user.json', 'utf8'));
  ...
  test('Check user profile @e2e', async ({ webApp }) => {
    const user = JSON.parse(fs.readFileSync('user.json', 'utf8'));
