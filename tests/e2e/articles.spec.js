...
test.describe('Articles test for realworld', () => {
  test('Create new article - positive @e2e', async ({ webApp, user}) => {
  ...
  test('Create new article - negative @e2e', async ({ webApp, user, article }) => {
    await webApp.page.waitForLoadState('networkidle');
    await webApp.mainPage.clickOnNewArticle();
    await webApp.articlePage.fillArticleForm(article);
    await expect(webApp.articlePage.errorMessage).toBeVisible();
  });

  test('Popular tags should filter articles correctly @e2e', async ({ webApp, user }) => {
